    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.variations.update", [$variation->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $variation->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" type="text" name="type" id="type" value="{{ old('type', $variation->type) }}" required>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $variation->price) }}" step="1" required>
                @if($errors->has('price'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sku') ? 'is-invalid' : '' }}" type="text" name="sku" id="sku" value="{{ old('sku', $variation->sku) }}">
                @if($errors->has('sku'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $variation->order) }}" step="1">
                @if($errors->has('order'))
